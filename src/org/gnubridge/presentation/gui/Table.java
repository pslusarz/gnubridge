...
public class Table {
	...
	public Table(int parentHeight) {
		dimensions = new Rectangle(290, parentHeight - CardPanel.IMAGE_HEIGHT
				- 35 - 5 - HEIGHT, 222, HEIGHT);
		currentTrickCards = new ArrayList<CardPanel>();
	}

	public void draw(Graphics g) {
		g.drawRect((int) dimensions.getX(), (int) dimensions.getY(),
	...
	public Point getExpectedSlot(Direction d) {
		...
		if (slot.equals(South.i())) {
			return new Point((int) (dimensions.getX() + dimensions.getWidth()
					/ 2 - CardPanel.IMAGE_WIDTH / 2), (int) (dimensions.getY()
					+ dimensions.getHeight() - CardPanel.IMAGE_HEIGHT));
		} else if (slot.equals(West.i())) {
			return new Point((int) dimensions.getX(), (int) (dimensions.getY()
					+ dimensions.getHeight() / 2 - CardPanel.IMAGE_HEIGHT / 2));
		} else if (slot.equals(North.i())) {
			return new Point((int) (dimensions.getX() + dimensions.getWidth()
					/ 2 - CardPanel.IMAGE_WIDTH / 2), (int) dimensions.getY());
		} else if (slot.equals(East.i())) {
			return new Point(
					(int) (dimensions.getX() + dimensions.getWidth() - CardPanel.IMAGE_WIDTH),
					(int) (dimensions.getY() + dimensions.getHeight() / 2 - CardPanel.IMAGE_HEIGHT / 2));
		}
		return null;
	}

	public void drawPromptArrow(Graphics g,
			Direction whereIsNextPlayer, boolean isCardBeingPlayed) {
	...
	public boolean contains(CardPanel card) {
		return dimensions.contains(card.getBounds().getCenterX(), card
	...
	void displayTrick(Trick trick, Container thePanel) {
